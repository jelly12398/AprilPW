...
Error: expect(Buffer).toMatchSnapshot(expected) failed
  ...
  Expected an image 1280px by 720px, received 311px by 63px. 190662 pixels (ratio 0.21 of all image pixels) are different.
...
```ts
  ...
  4  | await page.goto("https://demowebshop.tricentis.com/register")
  5  |  //expect(await page.screenshot()).toMatchSnapshot('home.png') //approach1
  6  |  //await expect(page).toHaveScreenshot()//approach2
  7  |  //specific element
  8  |  const logo= page.getByRole("link",{name:'Tricentis Demo Web Shop'})
> 9  |  expect(await logo.screenshot()).toMatchSnapshot()
     |                                  ^ Error: expect(Buffer).toMatchSnapshot(expected) failed
  10 | 
  11 | })
```